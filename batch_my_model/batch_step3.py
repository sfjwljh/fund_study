...
input_file=os.path.join(BASE_DIR,r'batch_my_model/0327第一批1952篇_补充好_step2_整合.json')
output_file=os.path.join(BASE_DIR,r'batch_my_model/0327第一批1952篇_补充好_step3.json')
    ...
    for i,part in enumerate(content):
        ...
        for industry in part['label_tree'].keys():
            ...
            for aspect in part['label_tree'][industry].keys():
                ...
                # 判断不在已经生成的里面
                task={'doc_number':doc_number,'sentence_number':i,'industry':industry,'aspect':aspect,'prompt':prompt}
                done_flag=False
                for done_task in query_done:
                    if done_task['doc_number']==task['doc_number'] and done_task['sentence_number']==task['sentence_number'] and done_task['industry']==task['industry'] and done_task['aspect']==task['aspect']:
                        done_flag=True
                        break
                if done_flag==False:
                    # 如果没有被处理过，再加入
...
def process_query(query):
    ...
    if result:
        return {'doc_number': query['doc_number'], 'sentence_number': query['sentence_number'], 'industry': query['industry'],'aspect':query['aspect'], 'prompt': query['prompt'], 'response': result}
    else:
        print(f"Error processing query: {query}")
        return None

model = Qianfan('d2x2wb8l_ljhs3_glm9b1')
