...
if __name__ == "__main__":
    ...
    db = pymysql.connect(host='bj-cynosdbmysql-grp-igalwqqk.sql.tencentcdb.com',
                            ...
                            port=25445,)
    cursor = db.cursor()
    select_query = "select code,m3u8_url from total where date>='2024-1-18' ORDER BY `date` DESC " # 选择一个没被下载过且不是正在被占用的
    ...
    for line in db_code_list:
        ...
        try:
            working_url=line[1]
            if working_url==None or len(working_url)==0:
                continue
        except:
            continue
        if not os.path.exists("C:\\Users\\LJH\\Desktop\\txt存放\\飞书\\{}.txt".format(working_code)):
...
if __name__ == "__main__":
    ...
    db = pymysql.connect(host='bj-cynosdbmysql-grp-igalwqqk.sql.tencentcdb.com',
                            ...
                            port=25445,)
    cursor = db.cursor()
    select_query = "select code,m3u8_url from total where date>='2023-11-10'  and date<='2023-12-11' ORDER BY `date` DESC; " # 选择一个没被下载过且不是正在被占用的
    cursor.execute(select_query)
    db_code_list=cursor.fetchall()

    for line in db_code_list:
        working_code=line[0]
        if os.path.exists("E:\\obsidian\\Master\\fund_stream_project\\codes\\tmp_ignore_sync\\{}.mp3".format(working_code)):
            continue
        try:
            working_url=line[1]
            if working_url==None or len(working_url)==0:
